...
element that contains the string. Also, Structure are listed with the brace
characters noted.
...
(Although, it the newlines for your file are wrong, PPI will probably have
localised them)
...
The following provides a list of all the primary classes contained in the
core PPI distribution. The list is in alphabetical order. Anything with it's
...
Please note that this does B<not> contain the 50 or so classes of the PDOM,
but only the "main players" within it.
